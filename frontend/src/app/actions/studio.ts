...
export async function cancelBookingAction(lessonId: string, email: string) {
  ...
  const hoursLeft = (new Date(lesson.start_timestamp).getTime() - Date.now()) / (1000 * 60 * 60)
  if (hoursLeft < 2) throw new Error('Cancellation window closed: less than 2 hours before start')
